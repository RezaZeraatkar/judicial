...
// Variables
const ToolBarHeaderText = "لیست انواع تذکرات";
const Height = 500;
...
export default function ApplauseTypesList(props) {
  ...
  // request to get rows for table
  const [addRemarkType, { isLoading }] = useAddRemarkTypesMutation();
  const [updateRemarkType] = useUpdateRemarkTypeMutation();
  const [deleteRemarkType] = useDeleteRemarkTypeMutation();
  ...
  const handleDeleteRecord = async (id) => {
    try {
      const deletedRemarkType = await deleteRemarkType(removedItemId).unwrap();
      setDeleteDialog(false);
      toast.success(deletedRemarkType.message, { position: "top-right" });
    } catch (error) {
      handleServerErrors(error);
    }
  };
  ...
  const handleAddRecord = async (record) => {
    try {
      const remarkType = await addRemarkType(record).unwrap();
      toast.success(remarkType.message, { position: "top-right" });
    } catch (error) {
      handleServerErrors(error);
    }
  };
  ...
  function computeMutation(newRow, oldRow) {
    if (newRow.description !== oldRow.description) {
      return `عنوان تذکر از '${oldRow.description}' به '${newRow.description}'`;
    }
    if (newRow.code !== oldRow.code) {
      return `کد تذکر از '${oldRow.code}' به '${newRow.code}'`;
    }
    return null;
  }
  ...
  const handleYes = async () => {
    ...
    try {
      ...
      const remarkType = await updateRemarkType(newRow).unwrap();
      setPromiseArguments(null);
      toast.success(remarkType.message, { position: "top-right" });
    } catch (error) {
      setPromiseArguments(null);
