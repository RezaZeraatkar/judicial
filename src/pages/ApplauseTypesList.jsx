...
// Variables
const ToolBarHeaderText = "لیست انواع تشویقات";
const Height = 500;
...
function ApplausesTypesForm({ cols, handleClick, isLoading }) {
...
export default function ApplauseTypesList(props) {
  ...
  // request to get rows for table
  const [createApplauseType, { isLoading }] = useCreateApplauseTypeMutation();
  const [deleteApplauseType] = useDeleteApplausesTypesMutation();
  const [updateApplauseType] = useUpdateApplauseTypeMutation();
  ...
  if (isSuccess) {
    rows = applausesTypesData.data?.map((at, index) => ({
      id: at.id,
      row: `${index + 1}`,
      code: `${at.code}`,
  ...
  const handleDeleteRecord = async (id) => {
    try {
      const deletedApplauseType = await deleteApplauseType(
        removedItemId,
      ).unwrap();
      setDeleteDialog(false);
      toast.success(deletedApplauseType.message, { position: "top-right" });
    } catch (error) {
      handleServerErrors(error);
    }
  };
  ...
  const handleAddRecord = async (record) => {
    try {
      const applauseType = await createApplauseType(record).unwrap();
  ...
  function computeMutation(newRow, oldRow) {
    if (newRow.description !== oldRow.description) {
      return `عنوان تشویق از '${oldRow.description}' به '${newRow.description}'`;
    }
    if (newRow.code !== oldRow.code) {
      return `کد تشویق از '${oldRow.code}' به '${newRow.code}'`;
    }
    return null;
  }
  ...
  const handleYes = async () => {
    ...
    try {
      ...
      const applauseType = await updateApplauseType(newRow).unwrap();
      setPromiseArguments(null);
      toast.success(applauseType.message, { position: "top-right" });
    } catch (error) {
      setPromiseArguments(null);
