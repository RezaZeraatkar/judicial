...
// Variables
const ToolBarHeaderText = "لیست حقوقی‌ها";
const Height = 500;
...
export default function HoghoughiList(props) {
  ...
  const {
    data: hoghughiOrganizationsData,
    isSuccess,
    isFetching,
    isError,
    error,
  } = useGetHoghughiOrganizationsQuery();
  const [addHoghoughi, { isLoading }] = useAddHoghoughiMutation();
  const [updateHoghoughi] = useUpdateHoghoughiMutation();
  const [deleteHoghoughi] = useDeleteHoghoughiMutation();
  ...
  if (isSuccess) {
    rows = hoghughiOrganizationsData.data?.map((el, index) => ({
      id: el.id,
      row: `${index + 1}`,
  ...
  const handleDeleteRecord = async (id) => {
    try {
      const deletedhoghoughi = await deleteHoghoughi(removedItemId).unwrap();
      setDeleteDialog(false);
      toast.success(deletedhoghoughi.message, { position: "top-right" });
    } catch (error) {
      handleServerErrors(error);
    }
  };
  ...
  const handleAddRecord = async (record) => {
    try {
      const hoghoughi = await addHoghoughi(record).unwrap();
      toast.success(hoghoughi.message, { position: "top-right" });
    } catch (error) {
      handleServerErrors(error);
    }
  };
  ...
  function computeMutation(newRow, oldRow) {
    if (newRow.description !== oldRow.description) {
      return `عنوان سازمان از '${oldRow.description}' به '${newRow.description}'`;
    }
    if (newRow.code !== oldRow.code) {
      return `کد سازمان از '${oldRow.code}' به '${newRow.code}'`;
    }
    return null;
  }
  ...
  const handleYes = async () => {
    ...
    try {
      ...
      const hoghoughi = await updateHoghoughi(newRow).unwrap();
      setPromiseArguments(null);
      toast.success(hoghoughi.message, { position: "top-right" });
    } catch (error) {
      setPromiseArguments(null);
